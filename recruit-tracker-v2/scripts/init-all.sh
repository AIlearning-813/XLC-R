...
#   1. 创建必备集合（如不存在）
#   2. 部署安全规则
#   3. 输出数据库索引控制台清单（**索引只能在控制台手建，本步不会真的创建**）
#   4. 初始化默认管理员和招聘专员账号
#   5. 写入默认系统配置
#   6. 部署全部云函数
#   7. 冒烟测试
#
# ⚠️ 索引不是自动的：SDK 与 CLI 都没有创建索引的能力（详见 scripts/deploy-indexes.cjs 头部）。
#    步骤 3 打印清单后，需人工去控制台逐条创建；脚本结束时会再次提醒。
#
# 使用方式：
...
# ===== 步骤 3：输出索引控制台清单 =====
# 这一步**不会**创建索引（SDK/CLI 均无此能力），只打印清单 + 校验与文档是否一致。
# 有意不让 `set -e` 在这里中断：索引是人工步骤，不该阻断后续的账号初始化与函数部署；
# 但校验失败必须显眼，故记下退出码留到最后统一提醒。
echo "📊 步骤 3/7：输出数据库索引控制台清单（需人工在控制台创建）..."
INDEX_CHECK_OK=1
node "$SCRIPT_DIR/deploy-indexes.cjs" || INDEX_CHECK_OK=0
echo ""
...
echo ""
echo "  1) 【必做】到控制台创建索引"
echo "     路径：控制台 → 数据库 → 集合 → 索引管理 → 新建索引"
echo "     清单见上方步骤 3 的输出（共 31 条，方向务必照抄）。"
echo "     未建索引不会报错，只会让列表查询退化为全表扫描。"
if [ "$INDEX_CHECK_OK" = "0" ]; then
  echo "     ❌ 注意：步骤 3 的交叉校验**失败**——deploy-indexes.cjs 与"
  echo "        scripts/init-database.md 的索引清单不一致，请先对齐两者。"
fi
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
